...
#         P = pts_flt.shape[0]
#         # print(pts_flt)
#         (dists, inds) = self.nbrs.kneighbors(pts_flt, n_neighbors=10)
#
...
#     alpha = 1.0
#
#     pc = PriorFactor(alpha, gamma, priorFile=os.path.join(enc_dir, 'prior_probs_bak.npy'))
#
...
def preprocess(data):
    ...
    # ab: [-110, 110]
    data_ab = img_lab[:, :, :, 1:]
    # data_ab = resize(data_ab, (int(H / 2), int(W / 2)))
    # 还得将缩放的ab值 归一化到0-1的区间 便于和output进行loss
    # data_ab = np.dot(data_ab, 1/128)

    # scale img_l to [-50, 50]
    data_l = img_l - 50
    data_ab_ss = data_ab[:, ::2, ::2, :]
    ...
    # NonGrayMask {N, 1, 1, 1}
    # thresh = 5
    # nongray_mask = (np.sum(np.sum(np.sum(np.abs(data_ab_ss) > thresh, axis=1), axis=1), axis=1) > 0)[:, np.newaxis,
...
def decode(data_l, con_output):
    """
    Args:
      data_l   : [1, height, width, 1]
      con_output: [1, height/2, width/2, 2]
    Returns:
      img_rgb  : [height, width, 3]
    """
    _, height, width, _ = data_l.shape
    data_l = data_l[0, :, :, :]
    data_ab = resize(con_output[0], (height, width))
