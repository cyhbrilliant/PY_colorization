...
saver.restore(sess,'color_session/session_CHA_1.ckpt')



err0,error,outfinal=sess.run([loss0,loss,OUT_final],feed_dict={imageDATA:tempimg_lab})
img_testab=np.reshape(outfinal,newshape=[H,W,1])
...
img_testa=img_testab-128
img_testab=np.concatenate((img_testab-128,tempimg_lab[0,:,:,2][:,:,np.newaxis]-128),axis=2)
...
# img_color=np.concatenate(((np.ones([H,W])*100)[:,:,np.newaxis],img_testac[:,:,0:1],(np.ones([H,W])*0)[:,:,np.newaxis]),axis=2)
img_color=np.concatenate(((np.ones([H,W])*100)[:,:,np.newaxis],img_testa,(np.ones([H,W])*0)[:,:,np.newaxis]),axis=2)
