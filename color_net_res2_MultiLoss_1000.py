...
file = h5py.File('imagenet_Labels_small.h5','r')
InputAll = file['Input'][:]
LabelAll=file['Label'][:]
...
low_Wconv2=weight_variable([3, 3, 64, 64])
low_Bconv2=bias_variable([64])
low_Wconv3=weight_variable([3, 3, 64, 64])
low_Bconv3=bias_variable([64])
low_Wconv4=weight_variable([3, 3, 64, 128])
low_Bconv4=bias_variable([128])
low_Wconv5=weight_variable([3, 3, 128, 128])
low_Bconv5=bias_variable([128])
low_Wconv6=weight_variable([3, 3, 128, 128])
low_Bconv6=bias_variable([128])
low_Wconv7=weight_variable([3, 3, 128,256])
low_Bconv7=bias_variable([256])
low_Wconv8=weight_variable([3, 3, 256, 256])
low_Bconv8=bias_variable([256])
low_Wconv9=weight_variable([3, 3, 256, 512])
low_Bconv9=bias_variable([512])
low_Wconv10=weight_variable([3, 3, 512, 512])
low_Bconv10=bias_variable([512])

#low_level1
CH1_conv1=tf.nn.relu(batch_norm(tf.nn.conv2d(XpVGG, low_Wconv1, strides=[1, 2, 2, 1], padding='SAME') + low_Bconv1,BNtrain))
CH1_conv2=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv1, low_Wconv2, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv2,BNtrain))
CH1_conv3=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv2, low_Wconv3, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv3,BNtrain))
CH1_conv4=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv3, low_Wconv4, strides=[1, 2, 2, 1], padding='SAME') + low_Bconv4,BNtrain))
CH1_conv5=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv4, low_Wconv5, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv5,BNtrain))
CH1_conv6=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv5, low_Wconv6, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv6,BNtrain))
CH1_conv7=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv6, low_Wconv7, strides=[1, 2, 2, 1], padding='SAME') + low_Bconv7,BNtrain))
CH1_conv8=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv7, low_Wconv8, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv8,BNtrain))
CH1_conv9=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv8, low_Wconv9, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv9,BNtrain))
CH1_conv10=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv9, low_Wconv10, strides=[1, 1, 1, 1], padding='SAME') + low_Bconv10,BNtrain))
...
#Global
global_conv1=tf.nn.relu(batch_norm(tf.nn.conv2d(CH1_conv10,global_Wconv1,strides=[1,2,2,1],padding='SAME')+global_Bconv1,BNtrain))
global_conv2=tf.nn.relu(batch_norm(tf.nn.conv2d(global_conv1,global_Wconv2,strides=[1,1,1,1],padding='SAME')+global_Bconv2,BNtrain))
global_conv3=tf.nn.relu(batch_norm(tf.nn.conv2d(global_conv2,global_Wconv3,strides=[1,2,2,1],padding='SAME')+global_Bconv3,BNtrain))
global_conv4=tf.nn.relu(batch_norm(tf.nn.conv2d(global_conv3,global_Wconv4,strides=[1,1,1,1],padding='SAME')+global_Bconv4,BNtrain))
...
varlist=[Wconv1,Bconv1,Wconv2,Bconv2,global_Wfc6,global_Bfc6,global_Wfc7,global_Bfc7]
...
for reslayer in range(5):
    ...
    varlist.append(weightRes3)
    varlist.append(biasRes3)
...
varlist.append(col_Wfu)
varlist.append(col_Bfu)
varlist.append(col_Wconv2)
varlist.append(col_Bconv2)
...
Wconv2_k=weight_variable([3,3,256,2])
Bconv2_k=bias_variable([2])
OUT=tf.nn.sigmoid(tf.nn.conv2d(col_conv2,Wconv2_k,strides=[1,1,1,1],padding='SAME')+Bconv2_k)
varlist.append(Wconv2_k)
varlist.append(Bconv2_k)
# print(OUT)

# loss=tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=Yp,logits=OUT))
loss=tf.reduce_mean(tf.square(Yp-OUT))
TrainStep=tf.train.AdamOptimizer(0.0001).minimize(loss,var_list=varlist)
loss2=tf.reduce_mean(-tf.reduce_sum(Ypclassfi*tf.log(tf.clip_by_value(classfi_fc2,1e-10,1.0)),reduction_indices=[1]))
TrainStep2=tf.train.AdamOptimizer(0.001).minimize(loss2)
...
# saver.restore(sess,'./models/session_res_multiloss2.ckpt')
...
for iter in range(1000000):
    ...
    #     index = np.random.randint(0, 800000)
    #     # print(index)
    #     Label_temp = np.zeros(65)
    #     Label_temp[LabelAll[index]] = 1
    #     loss2_, classfi_fc2_=sess.run([loss2, classfi_fc2],feed_dict={imageDATA:InputAll[index:index+1,:,:,:], Ypclassfi:Label_temp[np.newaxis,:]})
    #     classfi_fc2_sort = classfi_fc2_.sort()
    #     print(valerror, loss2_, classfi_fc2_sort[0:6])
    #     # print(outpoint[0,:])
    if (iter+1)%1000==0:
        path = saver.save(sess,'./models/session_res_multiloss3.ckpt')
        print(path)
